...
func example() {
	...
	fmt.Printf("Thank you for your order, %s!\n", name)
	fmt.Printf("Here’s what you ordered:\n")
	fmt.Printf("- Burger: %s\n", burger)
	fmt.Printf("- Toppings: %v\n", toppings)
	fmt.Printf("- Charm Sauce level: %d\n", sauceLevel)
	if instructions != "" {
		fmt.Printf("- Special Instructions: %s\n", instructions)
	}
	if discount {
		fmt.Printf("You’ve received 15%% off your order!\n")
	}
